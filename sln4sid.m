function [sys,K,rcnd,R] = sln4sid(s,y,u,n,R,tol,printw)
...
% 
%        SYS = SLN4SID(S,Y,U,N,ALG,TOL,PRINTW)  computes a state-space
...
%
%        [SYS,K,RCND] = SLN4SID(S,Y,U,N,ALG,TOL,PRINTW)  also returns the Kalman
%        predictor gain, as well as the vector RCND of length 14 containing
...
%        Riccati equation solutions. RCND(14) is an estimate of the forward
...
%        the N4SID identification technique (S > N).
%
...
%        Default:    PRINTW = 0.
%
%        [SYS,K] = SLN4SID(S,Y)  identifies a stochastic system without inputs,
%        the order N being prompted to be set by the user.
%        [SYS,K] = SLN4SID(S,Y,[],N)  identifies a stochastic system, the
%        order N being specified.
%
%        [SYS,K,RCND,R] = SLN4SID(S,Y,U,N,ALG,TOL,PRINTW)  also returns the 
...
%        inside the loop. RCND(1) and RCND(2) are set to 1 when SLN4SID is 
%        called with R instead of ALG.
%
%        See also ORDER, SIDENT, SLMOEN4, SLMOESM, SLMOESP
%
...
if numel( R ) == 1,
   ...
   [R,na,sval,rcndl] = findR(s,y,u,meth,R,jobd,tol,printw);
   if n < 0,
