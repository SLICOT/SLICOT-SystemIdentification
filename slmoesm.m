function [sys,K,rcnd,x0,R] = slmoesm(s,y,u,n,R,tol,printw)
...
%         and system simulation techniques: A and C are found via MOESP,
%         B and D via simulation.
% 
%        SYS = SLMOESM(S,Y,U,N,ALG,TOL,PRINTW)  computes a state-space
...
%
%        [SYS,K,RCND] = SLMOESM(S,Y,U,N,ALG,TOL,PRINTW)  also returns the Kalman
%        predictor gain, as well as the vector RCND of length 13 or 14 containing
...
%
%        [SYS,K,RCND,X0] = SLMOESM(S,Y,U,N,ALG,TOL,PRINTW)  also returns an 
%        estimate of the initial state X0.
%
...
%        Default:    PRINTW = 0.
%
%        [SYS,K] = SLMOESM(S,Y)  identifies a stochastic system without inputs,
%        the order N being prompted to be set by the user.
%        [SYS,K] = SLMOESM(S,Y,[],N)  identifies a stochastic system, the 
%        order N being specified.
%
%        [SYS,K,RCND,X0,R] = SLMOESM(S,Y,U,N,ALG,TOL,PRINTW)  also returns the 
...
%        following commands:
%
%        [SYS,K,RCND,X0,R] = SLMOESM(S,Y,U,N0,ALG,TOL,PRINTW);
%        for N = N0+1 : min( N0+Nf, S-1 )
%          [SYS,K,RCND,X0] = SLMOESM(S,Y,U,N,R,TOL,PRINTW);
%        end
...
%        inside the loop. 
%
%        See also ORDER, SIDENT, SLMOEN4, SLMOESP, SLN4SID
%
...
%
%        Comments:
%        1. This m-function could be less efficient, but more accurate
%           than SLMOEN4, SLMOESP, or SLN4SID.
%
...
% Use combined method: first MOESP and then simulation.
meth = 1;  meth2 = 2;  job2 = 1;
%
nsmpl = t - 2*s + 1;
...
%      
% Compute system matrices and x0.
withx0 = 1;  withoutx0 = 0;  withd = 1;
[A,C] = findAC(s,n,l,R,meth,tol(1),printw);
if job == 2,
   if nout == 2,
      [B,K] = findBDK(s,n,l,R,A,C,meth2,job2,nsmpl,tol(1),printw);   
   elseif nout == 3,
      [B,K,Q,Ry,S,rcnd] = findBDK(s,n,l,R,A,C,meth2,job2,nsmpl,tol(1),printw);   
      rcnd = [rcnd; ones(2,1)];    
   elseif nout >= 4 && nout <= 5,
      [B,K,Q,Ry,S,rcnd] = findBDK(s,n,l,R,A,C,meth2,job2,nsmpl,tol(1),printw);   
      [x0,V,rcndl] = findBD(withx0,3,A,C,y,tol(1),printw);
      rcnd = [rcnd; rcndl; 1];  
   end
   if nout < 1,
   ...
   if nout == 1,
      [B,D] = findx0BD(A,C,y,u,withoutx0,withd,tol(1),printw);   
   elseif nout == 2,
      [B,K] = findBDK(s,n,l,R,A,C,meth2,job2,nsmpl,tol(1),printw);   
      [B,D] = findx0BD(A,C,y,u,withoutx0,withd,tol(1),printw);   
   elseif nout == 3,
      [B,K,Q,Ry,S,rcnd] = findBDK(s,n,l,R,A,C,meth2,job2,nsmpl,tol(1),printw);   
      [B,D,V,rcndl]     = findx0BD(A,C,y,u,withoutx0,withd,tol(1),printw);   
      rcnd = [rcnd; rcndl];    
   elseif nout >= 4 && nout <= 5,
      [B,K,Q,Ry,S,rcnd] = findBDK(s,n,l,R,A,C,meth2,job2,nsmpl,tol(1),printw);   
      [x0,B,D,V,rcndl]  = findx0BD(A,C,y,u,withx0,withd,tol(1),printw);   
      rcnd = [rcnd; rcndl];    
   elseif nout < 1,
      error('Wrong number of output arguments')
   end   
