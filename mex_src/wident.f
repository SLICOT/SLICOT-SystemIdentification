C WIDENT.F - Gateway function for computation of a discrete-time
C            model of a Wiener system using SLICOT routine IB03BD.
C
C RELEASE 2.0 of SLICOT System Identification Toolbox.
C Based on SLICOT RELEASE 5.7. Copyright (c) 2002-2020 NICONET e.V.
C
C Matlab calls:
C   [xopt(,perf,nf,rcnd)] = wident(job,u,y,nn(,s,n,x,iter,nprint,tol,
C                                  seed,printw,ldwork))
C
C   [xopt(,perf,nf,rcnd)] = wident(1,u,y,nn,s,n,x(,iter,nprint,tol,
C                                  printw,ldwork))
C   [xopt(,perf,nf)]      = wident(2,u,y,nn,n,x(,iter,nprint,tol,
C                                  seed,printw,ldwork))
C   [xopt(,perf,nf,rcnd)] = wident(3,u,y,nn,s(,n,[],iter,nprint,tol,
C                                  seed,printw,ldwork))
C   [xopt(,perf,nf)]      = wident(4,u,y,nn,n,x(,iter,nprint,tol,
C                                  printw,ldwork))
C
C Purpose:
...
C   MINPACK-like Levenberg-Marquardt algorithm.
C
C Input parameters:
...
C            intermediate results are printed in the file IB03BD.prn,
C            which is overwritten at each execution of this mexfile.
C            Default: nprint = 0 (no printing).
C   tol    - (optional) vector of length 2 containing the absolute
C            tolerances for the initialization of the static
C            nonlinearity (ignored if job = 1 or 4), and for the
C            optimization process, respectively.
...
C   perf   - (optional) vector of length 4 or 8 containing performance
C            results:
C            perf(1) contains the optimal value of the workspace length;
C            perf(2) contains the maximum residual error norm;
C            perf(3) contains the total number of iterations performed;
C            perf(4) contains the final Levenberg factor.
C            If job = 2 or 3, then similar results for the nonlinear
C            part initialization step are returned:
C            perf(5) contains the optimal value of the workspace length;
C            perf(6) contains the maximum residual error norm;
C            perf(7) contains the total number of iterations performed;
C            perf(8) contains the final Levenberg factor (maximum over
C                    all outputs).
...
C
C Revisions:
C   V. Sima, Katholieke University Leuven, Belgium, Feb. 2002,
C   Apr. 2009, Dec. 2012.
C
C **********************************************************************
C
...
C .. Local variables and constant dimension arrays ..
      ...
      INTEGER           BSN, INI, IP, ISAD, ISIZE, ITMP, IW1, IW2, IW3,
     $                  IX, JWORK, L1, L2, LDAC, LDWMIN, LIWORK, LNOL,
     $                  LPAR, LPAR0, M1, M2, ML, MNO, N2, NS, NSML, NX,
     $                  TASK
...
C
      IF ( NRHS.LT.4 ) THEN
         CALL mexErrMsgTxt
     $        ( 'WIDENT requires at least 4 input arguments' )
      ELSE IF ( NLHS.LT.1 ) THEN
         CALL mexErrMsgTxt
     $        ( 'WIDENT requires at least 1 output arguments' )
      END IF
C
C   job, u(t*m), y(t*l), nn, s, n, x(lx0), iter(2), nprint, tol(2),
C   seed, printw, ldwork
C
...
C
     ...
     $        mxIsComplex( PRHS(IP) ).EQ.1 ) THEN
      ...
      ELSE IF ( .NOT. INIT1 ) THEN
         WRITE( TEXT, '('' WIDENT requires at least '',I4,
     $                  '' input arguments'')' ) IP
...
C
     ...
     $        mxIsComplex( PRHS(IP) ).EQ.1 ) THEN
         ...
         IF ( NPRINT.GT.0 )
     $      OPEN( NOUT, file = 'IB03BD.prn', status = 'replace' )
         IP = IP + 1
      END IF
C
...
C     Workspace for MD03BD (whole optimization).
      ...
      ELSE
         IW1 = L
      END IF
      JWORK = MAX( N*LDAC, IW1, JWORK )
      IW1   = NSML + MAX( 2*NN, ISAD + 2*N + JWORK )
      IF ( L.LE.1 .OR. BSN.EQ.0 ) THEN
         IW3 = 4*LX
         IW2 = IW3 + 1
      ELSE
         IW2 = BSN + MAX( 3*BSN + 1, L2 )
         IF ( NSMP.GT.BSN ) THEN
            IW2 = MAX( IW2, 4*L2 + 1 )
            IF ( NSMP.LT.2*BSN )
     $         IW2 = MAX( IW2, ( NSMP - BSN )*( L - 1 ) )
         END IF
         IW3 = L2*BSN + 2*LX + 2*MAX( BSN, L2 )
      END IF
      JWORK = NSML + LX +
     $        MAX( 4, NSML +
     $                MAX( NSML*( BSN + L2 ) +
...
C
     ...
     $   LDWORK = LDWMIN
      ...
      END IF
      IF ( INIT2 ) THEN
         IW3 = 3 + MAX( BSN + 1, LX + L )
      ELSE
         IW3 = 3 + LX + L
      END IF
      LIWORK = MAX( IW1, IW2, IW3 )
C
...
C
      CALL IB03BD( INIT, NOBR, M, L, NSMP, N, NN, ITMAX1, ITMAX2,
...
C
      ...
      IF ( NLHS.GT.3 .AND. INIT1 ) THEN
         ...
         CALL mxCopyReal8ToPtr( DWORK(9), mxGetPr( PLHS(4) ), ISIZE )
      END IF
C
...
C
      IF ( IWARN.NE.0 .AND. PRINTW ) THEN
         WRITE( TEXT, '(''  IWARN = '',I4,'' ON EXIT FROM IB03BD'')'
     $        ) IWARN
         CALL mexPrintf( TEXT )
      END IF
C
      IF ( INFO.NE.0 ) THEN
         WRITE( TEXT, '('' INFO = '',I4,'' ON EXIT FROM IB03BD'')'
     $        ) INFO
         CALL mexErrMsgTxt( TEXT )
      END IF
C
      RETURN
C *** Last line of WIDENT ***
      END
