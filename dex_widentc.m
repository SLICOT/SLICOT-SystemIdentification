% File dex_widentc.m
%
% Script using the SLICOT mexfile widentc on a given example,
% for estimating the parameters of a Wiener system. 
% The linear part is estimated using SLICOT m-file slmoen4.
% If the option job of widentc is specified as 1, 2, or 4, and the 
...
% identical, since the computations inside and outside widentc 
...
%          (default 1000);
% ialg   - option specifying the algorithm for solving linear systems:
%          = 1 : Cholesky factorization (default);
%          = 2 : Conjugate Gradients;
% stor   - if ialg = 1, option specifying the storage scheme for the
%          symmetric matrix J'*J (J is the Jacobian matrix):
%          = 1 :  full storage is used(default);
...
if ~( exist( 'ialg', 'var' ) ) || isempty( ialg ),
   ialg = 1;
end   

if ialg == 1.
   if ~( exist( 'stor', 'var' ) ) || isempty( stor ),
      stor = 1;
   end
end   
...
disp( ' ' )
disp( 'Running widentc.  Please wait.' )
disp( ' ' )
...
disp( ' ')
if ialg == 1,
   disp( [ 'Algorithm option:  alg  = ',        num2str( ialg ),  ' (Cholesky algorithm) ' ] )
   disp( [ 'Storing   option:  stor = ',        num2str( stor ) ] )
else
   disp( [ 'Algorithm option:  alg  = ',        num2str( ialg ), ...
...
if ~( job == 3 ) && initx,
   ...
   if job == 1 || job == 4,
      ...
      disp( ' ' )
      if ialg == 1,
         [ xo, perfo, nfo ] = widentc( 2, u, y, nn, n, theta, ialg, stor, [ITMAX1, 0], ...
                                       nprint, [tol1, tol2], seed, printw );   
      else                          
         [ xo, perfo, nfo ] = widentc( 2, u, y, nn, n, theta, ialg, [ITMAX1, 0], ...
                                       nprint, [tol1, tol2], seed, printw );   
      end,                          
      x = xo;
      if printw,
...
tic
   ...
   if job == 1,
     if ialg == 1,
        [ xopt, perf, nf, rcnd ] = widentc( job, u, y, nn, s, n, x, ialg, stor, [ITMAX1, ITMAX2], ...
                                            nprint, [tol1, tol2], printw );   
     else
        [ xopt, perf, nf, rcnd ] = widentc( job, u, y, nn, s, n, x, ialg, [ITMAX1, ITMAX2], ...
                                            nprint, [tol1, tol2], printw );   
     end,
   elseif job == 2,
     if ialg == 1,
        [ xopt, perf, nf ] = widentc( job, u, y, nn, n, x, ialg, stor, [ITMAX1, ITMAX2], ...
                                     nprint, [tol1, tol2], seed, printw );   
     else
        [ xopt, perf, nf ] = widentc( job, u, y, nn, n, x, ialg, [ITMAX1, ITMAX2], ...
                                     nprint, [tol1, tol2], seed, printw );   
     end,
   elseif job == 3,
     if ialg == 1,
        [ xopt, perf, nf, rcnd ] = widentc( job, u, y, nn, s, n, [], ialg, stor, [ITMAX1, ITMAX2], ...
                                            nprint, [tol1, tol2], seed, printw );   
     else
        [ xopt, perf, nf, rcnd ] = widentc( job, u, y, nn, s, n, [], ialg, [ITMAX1, ITMAX2], ...
                                            nprint, [tol1, tol2], seed, printw );   
     end,
   else
     if ialg == 1,
...
if job == 2 || job == 3,
   ...
   disp( ['Sum of squares          :   ', sprintf( '%11.5g', perf(2) ),...
          '                    ',         sprintf( '%11.5g', perf(7) ) ] )
   disp( ['Number of iterations    :   ', sprintf( '%11i',   perf(3) ),...
          '                    ',         sprintf( '%11i',   perf(8) ) ] )
   disp( ['Number of CG iterations :   ', sprintf( '%11i',   perf(4) ),...
          '                    ',         sprintf( '%11i',   perf(9) ) ] )
   disp( ['Final Levenberg factor  :   ', sprintf( '%11.5g', perf(5) ),...
          '                    ',         sprintf( '%11.5g', perf(10) ) ] )
else
