function [sys,K,rcnd,R] = slmoen4(s,y,u,n,R,tol,printw)
...
%         techniques: A and C are found via MOESP, B and D via N4SID.
% 
%        SYS = SLMOEN4(S,Y,U,N,ALG,TOL,PRINTW)  computes a state-space
...
%
%        [SYS,K,RCND] = SLMOEN4(S,Y,U,N,ALG,TOL,PRINTW)  also returns the Kalman
...
%        the MOESP and N4SID identification techniques (S > N).
%
...
%        Default:    PRINTW = 0.
%
%        [SYS,K] = SLMOEN4(S,Y)  identifies a stochastic system without inputs,
%        the order N being prompted to be set by the user.
%        [SYS,K] = SLMOEN4(S,Y,[],N)  identifies a stochastic system, the 
%        order N being specified.
%
%        [SYS,K,RCND,R] = SLMOEN4(S,Y,U,N,ALG,TOL,PRINTW)  also returns the 
...
%        inside the loop. 
%
%        See also ORDER, SIDENT, SLMOESM, SLMOESP, SLN4SID
%
...
% Use combined method: first MOESP and then N4SID.
meth = 1;  meth2 = 3;
%
nsmpl = t - 2*s + 1;
...
if nout == 1,
   [A,C,B,D] = sident(meth2,job,s,n,l,R,tol(1),nsmpl,[],[],printw);
elseif nout == 2,
   [A,C,B,D,K] = sident(meth2,job,s,n,l,R,tol(1),nsmpl,[],[],printw);
elseif nout == 3 || nout == 4,
   [A,C,B,D,K,Q,Ry,S,rcnd] = sident(meth2,job,s,n,l,R,tol(1),nsmpl,[],[],printw);
elseif nout < 1,
